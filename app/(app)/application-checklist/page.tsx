...
export default function ChecklistPage() {
  const groups = mockChecklist.reduce<Record<string, typeof mockChecklist>>((acc, c) => {
                ...
                <li key={c.id} className="py-2 flex justify-between gap-3 items-start">
                  <div>
                    <div className="text-sm font-medium">
                      {c.label} {c.required ? <span className="text-red-600">*</span> : <span className="text-gray-400 text-xs">(optional)</span>}
                    </div>
                    {c.notes && <div className="text-xs text-gray-500">{c.notes}</div>}
                  </div>
                  <span className={
                    c.status === "complete" ? "badge-green"
                    : c.status === "uploaded" ? "badge-blue"
                    : "badge-amber"
                  }>
